...
SimulationApp* g_pApp = nullptr;

SimulationApp::SimulationApp(Context* context) : Application(context)
{
	g_pApp = this;
	m_bIsInit = false;
...
void SimulationApp::Setup()
{
	...
	WindowOptions& windowOptions = m_SimulationOptions.GetWindowsOptions();
...
void SimulationApp::Start()
{
    ...
    if (!m_SimulationOptions.InitSimulationOptions(String("System/SimulationOptions.xml")))
    {
    ...
    SubscribeToEvent(E_KEYDOWN, URHO3D_HANDLER(SimulationApp, KeyDownDelegate));
	SubscribeToEvent(E_KEYUP, URHO3D_HANDLER(SimulationApp, KeyUpDelegate));
	SubscribeToEvent(E_MOUSEMOVE, URHO3D_HANDLER(SimulationApp, MouseMoveDelegate));
	SubscribeToEvent(E_MOUSEBUTTONDOWN, URHO3D_HANDLER(SimulationApp, MouseDownDelegate));
	SubscribeToEvent(E_MOUSEBUTTONUP, URHO3D_HANDLER(SimulationApp, MouseUpDelegate));
	SubscribeToEvent(E_INPUTFOCUS, URHO3D_HANDLER(SimulationApp, InputFocusDelegate));
	SubscribeToEvent(E_UPDATE, URHO3D_HANDLER(SimulationApp, UpdateDelegate));
...
bool SimulationApp::CreateDebugHudAndConsole(XMLFile* style)
{
    // Create console
...
void SimulationApp::AbortGame()
{
	engine_->Exit();
}

void SimulationApp::InitInstance(int screenWidth, int screenHeight, bool windowMode, bool vsync, int multisample, bool triplebuffer)
{
...
bool SimulationApp::VLoadGame()
{
	// Read the game options and see what the current game
	// needs to be - all of the game graphics are initialized by now, too...
	return m_pGameLogic->VLoadGame(m_SimulationOptions.GetSceneName());
}

bool SimulationApp::OnMessageProc(AppMsg message)
{
...
// -----------------------------------------
void SimulationApp::KeyDownDelegate(StringHash eventType, VariantMap& eventData)
{
...
void SimulationApp::MouseMoveDelegate(StringHash eventType, VariantMap& eventData)
{
	AppMsg msg;
...
void SimulationApp::MouseDownDelegate(StringHash eventType, VariantMap& eventData)
{
	AppMsg msg;
...
void SimulationApp::MouseUpDelegate(StringHash eventType, VariantMap& eventData)
{
	AppMsg msg;
...
void SimulationApp::InputFocusDelegate(StringHash eventType, VariantMap& eventData)
{
...
void SimulationApp::KeyUpDelegate(StringHash eventType, VariantMap& eventData)
{
	using namespace KeyDown;
	// Check for pressing ESC. Note the ScanEngine_ member variable for convenience access to the ScanEngine object
	int key = eventData[P_KEY].GetInt();
...
void SimulationApp::UpdateDelegate(StringHash eventType, VariantMap& eventData)
{
